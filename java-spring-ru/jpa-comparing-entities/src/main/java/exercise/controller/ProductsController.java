...
public class ProductsController {
    ...
    public Product create(@RequestBody Product newProduct) {
        boolean exists = productRepository.findAll().stream()
                .anyMatch(product -> product.equals(newProduct));

        if (exists) {
            throw new ResourceAlreadyExistsException("Product with title " + newProduct.getTitle() + " and price " + newProduct.getPrice() + " already exists");
        }
